...
/*   Updated: 2023/03/11 01:26:12 by minjungk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init(struct s_simulator *simulator)
{
	...
	simulator->common.lock = ft_sem_open("common", 0);
	if (simulator->common.lock == SEM_FAILED)
		return (-1);
	num_of_philos = simulator->common.number_of_philosophers;
	i = -1;
	while (++i < num_of_philos)
	{
		simulator->forks[i].lock = ft_sem_open("fork", i);
		if (simulator->forks[i].lock == SEM_FAILED)
			return (-1);
	}
	i = -1;
	while (++i < num_of_philos)
...
extern void	finally(struct s_simulator *simulator, int is_philosopher)
{
	...
	sem_close(simulator->common.lock);
	if (is_philosopher == 0)
		ft_sem_unlink("common", 0);
	i = -1;
	while (++i < simulator->common.number_of_philosophers)
	{
		sem_close(simulator->forks[i].lock);
		if (is_philosopher == 0)
			ft_sem_unlink("fork", i);
	}
